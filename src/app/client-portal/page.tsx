...
import { BarChart3, CheckCircle2, Download, Eye, MapPin, ShieldCheck, Store, TrendingUp, Users, X } from "lucide-react";
...
export default function ClientPortalPage() {
  ...
  const activityTrend = useMemo(() => { const daily = new Map<string, { day: string; visits: number; checks: number }>(); activities.forEach(activity => { const entry = daily.get(activity.date) ?? { day: activity.date.slice(5), visits: 0, checks: 0 }; if (activity.type === "Store visit") entry.visits += 1; if (activity.type === "Product check") entry.checks += 1; daily.set(activity.date, entry); }); return [...daily.values()].sort((a, b) => a.day.localeCompare(b.day)); }, [activities]);
  ...
  return <AppShell>
    ...
    <section className="row charts-row client-top-row">
      <article className="card chart-clickable" role="button" tabIndex={0} onClick={() => openDetail("Field activity", `${activities.length} activities recorded for ${client.name}.`)}><div className="card-head"><div><h2>Field activity</h2><p>Visits and product checks over time</p></div><div className="legend"><span><i className="blue" />Visits</span><span><i className="teal" />Product checks</span></div></div><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><AreaChart data={activityTrend}><defs><linearGradient id="clientFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2563eb" stopOpacity={.2} /><stop offset="100%" stopColor="#2563eb" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)" /><XAxis dataKey="day" tick={{ fill: "var(--muted)", fontSize: 10 }} /><YAxis allowDecimals={false} tick={{ fill: "var(--muted)", fontSize: 10 }} /><Tooltip /><Area type="monotone" dataKey="checks" stroke="#14b8a6" strokeWidth={2} fill="transparent" /><Area type="monotone" dataKey="visits" stroke="#2563eb" strokeWidth={2.5} fill="url(#clientFill)" /></AreaChart></ResponsiveContainer></div></article>
