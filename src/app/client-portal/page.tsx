...
export default function ClientPortalPage() {
  ...
  return <AppShell>
    ...
    <section className="row charts-row client-top-row">
      <article className="card chart-clickable" role="button" tabIndex={0} onClick={() => openDetail("Activity breakdown", `${activities.length} activities recorded for ${client.name}, grouped by operational type.`)}><div className="card-head"><div><h2>Activity breakdown</h2><p>Work completed by activity type</p></div><div className="legend"><span><i className="blue" />Completed work</span></div></div><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={activityBreakdown} margin={{ top: 18, right: 10, left: 10, bottom: 18 }}><XAxis dataKey="name" tick={{ fill: "var(--muted)", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis hide allowDecimals={false} /><Tooltip /><Line type="monotone" dataKey="total" name="Activities" stroke="#2563eb" strokeWidth={3} dot={{ r: 4, fill: "#2563eb", stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 6 }} /></LineChart></ResponsiveContainer></div></article>
