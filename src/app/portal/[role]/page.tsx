...
const roleCopy: Record<AppRole, { title: string; subtitle: string; focus: string; icon: typeof Users }> = {
  ...
  admin: { title: "Admin control centre", subtitle: "Manage divisions, accounts, and outlet requests.", focus: "Governance & approvals", icon: Users },
  ...
  merchandiser: { title: "Merchandiser workspace", subtitle: "Capture store execution, products, and daily activity.", focus: "Today's field work", icon: Store },
  tsr: { title: "TSR workspace", subtitle: "Coordinate territories, teams, and performance.", focus: "Territory performance", icon: Users },
...
function RoleContent({ role, copy, user, navigate }: { role: AppRole; copy: typeof roleCopy[AppRole]; user: { name: string; email: string }; navigate: (path: string) => void }) {
  ...
  if (role === "supervisor") return <><section className="portal-hero"><div className="eyebrow"><span className="live-dot" /> SUPERVISOR ACCESS</div><h1>{copy.title}</h1><p>{copy.subtitle}</p><span className="portal-user">Signed in as {user.name} · {user.email}</span></section><section className="portal-workspace-grid"><article className="portal-focus"><Users size={24} /><small>TEAM OVERVIEW</small><h2>Open supervisor console</h2><p>Manage team overview, merchandiser performance, VSR performance, coaching, route coverage, data quality audits and settings.</p><button className="primary" onClick={() => navigate("/supervisor")}>Open supervisor dashboard</button></article><article className="portal-focus"><BarChart3 size={24} /><small>OPERATIONS</small><h2>View operations dashboard</h2><p>Access the operations overview with live map, workforce table and performance charts.</p><button className="secondary" onClick={() => navigate("/dashboard")}>Open operations</button></article></section></>;
