...
export default function SystemLogsPage() {
  ...
  return <AppShell contentClassName="page-shell"><PageHeading eyebrow="SYSTEM LOGS · PLATFORM HEALTH" title="System Logs" subtitle="Monitor platform services, request health, and operational exceptions." actions={<button className="secondary" type="button" onClick={reset}><RefreshCw size={15} /> Reset</button>} />
    <section className="filters"><label className="select-box"><span>LEVEL</span><select value={level} onChange={(event) => setLevel(event.target.value)}><option>All levels</option><option>INFO</option><option>WARN</option><option>ERROR</option></select></label><button className="reset" type="button" onClick={reset}><RefreshCw size={14} /> Reset filters</button></section>
    <section className="filters" style={{ marginTop: 8 }}><label className="select-box"><span>SERVICE</span><select value={service} onChange={(event) => setService(event.target.value)}><option>All services</option>{Array.from(new Set(logs.map((log) => log.service))).map((item) => <option key={item}>{item}</option>)}</select></label><label className="mini-search"><Search size={15} /><input placeholder="Search system events" value={query} onChange={(event) => setQuery(event.target.value)} /></label></section>
