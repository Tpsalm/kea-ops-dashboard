...
const roleData = [
  { name: "Merchandisers", value: 182, color: "#94C83D" }, { name: "VSRs", value: 96, color: "#7da830" },
...
export default function Dashboard() {
  ...
  return <div className={isDark ? "app dark" : "app"}>
    ...
    <aside className={mobileNav ? "sidebar open" : "sidebar"}>
      <div className="brand"><div className="brand-logo" aria-label="KEA Corporate Hospitality Services"><b className="logo-k">k</b><b className="logo-e">e</b><b className="logo-a">a</b><small>Corporate Hospitality Services</small></div><div><strong>Kea</strong><span>Talent Management System</span></div><button className="close-nav" onClick={()=>setMobileNav(false)}><X size={20}/></button></div>
    ...
    <main className="main">
      <header className="topbar"><button type="button" className="mobile-menu" onClick={()=>setMobileNav(true)} aria-label="Open navigation"><Menu size={21}/></button><div className="top-search"><Search size={17}/><input id="global-search" placeholder="Search people, stores, routes..." value={query} onChange={e=>{setQuery(e.target.value);setPage(1)}}/><kbd>⌘ K</kbd></div><div className="top-actions"><button type="button" onClick={toggleTheme} aria-label="Toggle dark mode" title={`Switch to ${isDark ? "Light" : "Dark"} mode`}>{isDark?<Sun size={19}/>:<Moon size={19}/>}</button><button type="button" className="bell" onClick={()=>setNotificationsOpen(!notificationsOpen)} aria-label="Open notifications"><Bell size={19}/><i/></button><button type="button" className="user-avatar small profile-button" onClick={()=>setPanel("profile")} aria-label="Open profile">KA</button></div>{notificationsOpen&&<div className="notification-popover"><div><b>Notifications</b><button type="button" onClick={()=>setNotificationsOpen(false)}><X size={15}/></button></div><p><span className="notice-dot"/> Lagos Central reached 92% coverage.</p><p><span className="notice-dot teal"/> 142 stores were added this month.</p><button type="button" onClick={()=>{setNotificationsOpen(false);flash("Notifications marked as read")}}>Mark all as read</button></div>}</header>

      <div className="content" id="overview">
        <section className="heading"><div><div className="eyebrow"><span className="live-dot"/> LIVE TALENT OPERATIONS · UPDATED 4 MIN AGO</div><h1>Kea Talent Management System</h1><p>Kea workforce, outlet, and field performance at a glance.</p></div></section>
        ...
        <section className="commerce-grid" id="commerce">
          {[
            { href: "/sales", label: "Sales", sub: "Revenue & volumes", icon: Wallet, color: "#94C83D" },
            { href: "/credits", label: "Credits", sub: "Loans & exposure", icon: Landmark, color: "#F37021" },
            { href: "/targets", label: "Targets", sub: "Attainment", icon: Target, color: "#7da830" },
            { href: "/outlets", label: "Outlets", sub: "Retail network", icon: Route, color: "#e05e12" },
            { href: "/visits", label: "Visits", sub: "Field execution", icon: CalendarCheck, color: "#84b832" },
        ...
        <section className="row charts-row" id="performance">
          <article className="card activity-card"><div className="card-head"><div><h2>Field activity</h2><p>Visits and product checks over time</p></div><div className="legend"><span><i className="blue"/>Visits</span><span><i className="teal"/>Product checks</span><button type="button" aria-label="Field activity options" onClick={()=>setPanel("activity")}><MoreHorizontal size={18}/></button></div></div><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartActivityData} margin={{top:10,right:8,left:-22,bottom:0}}><defs><linearGradient id="blueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#94C83D" stopOpacity={.22}/><stop offset="100%" stopColor="#94C83D" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)"/><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{fill:"var(--muted)",fontSize:11}}/><YAxis axisLine={false} tickLine={false} tick={{fill:"var(--muted)",fontSize:11}}/><Tooltip content={<Tip/>}/><Area type="monotone" dataKey="checks" stroke="#7da830" strokeWidth={2} fill="transparent"/><Area type="monotone" dataKey="visits" stroke="#94C83D" strokeWidth={2.5} fill="url(#blueFill)"/></AreaChart></ResponsiveContainer></div></article>
        ...
        <footer><span>Kea · Secure talent workspace</span><span><Zap size={13}/> All systems operational</span></footer>
      </div>
    </main>
    ...
    {panel && <div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setPanel(null)}}><section className={panel==="model"?"action-modal model-modal":"action-modal"} role="dialog" aria-modal="true" aria-label="Dashboard action panel"><div className="modal-head"><div><small>KEA OPERATIONS</small><h2>{panel==="data"?"Data quality":panel==="reports"?"Reports & exports":panel==="settings"?"Display settings":panel==="activity"?"Field activity options":panel==="workforce"?"Workforce mix options":panel==="completion"?"Visit completion options":panel==="filters"?"Advanced workforce filters":panel==="profile"?"Administrator profile":panel==="model"?"KEA operational data model":"Client dashboard"}</h2></div><button type="button" onClick={()=>setPanel(null)} aria-label="Close panel"><X size={19}/></button></div><div className="modal-body">
      ...
      {(panel==="settings" || panel==="profile")&&<ProfileSettingsModal
        isOpen={panel === "settings" || panel === "profile"}
        onClose={() => setPanel(null)}
        role="super-admin"
        defaultName="KEA Administrator"
        defaultEmail="admin@kea.com"
        roleLabel="Super Admin Executive · Full Access"
        onFlash={flash}
      />}
