-- =====================================================================
-- KEA Bidirectional Event-Driven Workflow & Tracker — Schema Migration
...
CREATE INDEX IF NOT EXISTS idx_workflows_supervisor_status
    ON workflows (assigned_supervisor_id, status);
CREATE INDEX IF NOT EXISTS idx_workflows_originator_time
    ON workflows (originator_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_workflows_admin_status
    ON workflows (assigned_admin_id, status);
...
CREATE INDEX IF NOT EXISTS idx_workflow_steps_workflow_time
    ON workflow_steps (workflow_id, occurred_at);
...
CREATE INDEX IF NOT EXISTS idx_wfmsg_workflow_sent
    ON workflow_messages (workflow_id, sent_at DESC);
CREATE INDEX IF NOT EXISTS idx_wfmsg_target_read
    ON workflow_messages (target_user_id, is_read);
...
-- workflows: SELECT — originator / assigned supervisor / assigned admin / TSR of supervisor
...
-- workflows: INSERT — creator is the originator, or supervisor/admin can insert
...
-- workflows: UPDATE — only supervisor/admin can transition status
...
-- workflow_steps: SELECT — same scope as workflows
...
-- workflow_steps: INSERT — app layer only, restrict to authenticated
...
-- workflow_messages: SELECT — sender or recipient only
...
-- workflow_messages: INSERT — sender must be auth.uid()
